    ...
    module LocaleHelpers
      ...
      def autofill(yaml)
          ...
          if i18n_value.is_a? Hash
            ...
            yaml_variable_regex = /\%\{[\w]+\}/
            yaml_arguments = i18n_value.scan(yaml_variable_regex).uniq

            next [i18n_key, i18n_value] if yaml_arguments.empty?

            yaml_arguments.each do |argument|
              non_word_character_regex = /[^\w]/
              method_name = argument.gsub(non_word_character_regex, "")

              begin
                i18n_value.gsub!(argument, self.public_send(method_name))
              rescue NoMethodError => error
                i18n_value.gsub!(argument, no_method_message(method_name))
              end
            end
